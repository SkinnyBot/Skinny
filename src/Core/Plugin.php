...
class Plugin
{
     ...
     * - `ignoreMissing` - boolean - Set to true to ignore missing commands files.
    ...
    public static function load($plugin, array $config = [])
    {
        ...
        if ($config['commands'] === true) {
            static::commands($plugin);
            Configure::load($plugin . '.commands');
        }
    }
     ...
     * @return mixed
     */
    public static function commands($plugin)
    {
        ...
        if ($config['commands'] === true) {
            return static::_includeFile(
                $config['configPath'] . 'commands.php',
